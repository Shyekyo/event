...
/**
  ...
  *ideal.Rep_total_account_commit
  */
object Rep_total_account_commit_BAK {
  def main(args: Array[String]): Unit = {
    Logger.getLogger("org").setLevel(Level.OFF)
    var prefix = "D:\\GITRepo\\event\\src\\resources"
    //prefix = "C:\\Users\\chendd\\Desktop\\张小凡\\LocalRepo\\event\\src\\"
    val account_type = prefix+"\\rep_total_account_commit\\account_type.csv"
    val acct_item = prefix+"\\rep_total_account_commit\\acct_item.csv"
    val acct_item_type = prefix+"\\rep_total_account_commit\\acct_item_type.csv"
    val amount = prefix+"\\rep_total_account_commit\\amount.csv"
    val billing_cycle = prefix+"\\rep_total_account_commit\\billing_cycle.csv"
    val gen_item = prefix+"\\rep_total_account_commit\\gen_item.csv"
    val stt_object = prefix+"\\rep_total_account_commit\\stt_object.csv"
    ...
    val acct_item_df = sc.textFile(acct_item).map(
      line => {
        ...
        val arr = rline.split("\\|")
        if(arr.length==19){
          CaseClass.ACCT_ITEM(arr(0).toInt, arr(1), arr(2).toInt, arr(3).toInt, arr(4).toInt, arr(5).toInt,
            arr(6).toInt, arr(7), arr(8), arr(9).toInt, arr(10).toInt, arr(11), arr(12), arr(13),
            arr(14).toInt, arr(15), arr(16), arr(17).toInt, arr(18).toInt)
        }else{
          CaseClass.ACCT_ITEM(arr(0).toInt, arr(1), arr(2).toInt, arr(3).toInt, arr(4).toInt, arr(5).toInt,
            arr(6).toInt, arr(7), arr(8), arr(9).toInt, arr(10).toInt, arr(11), arr(12), arr(13),
            arr(14).toInt, arr(15), arr(16), arr(17).toInt, 0)
        }
      }).toDS()
    ...
    val amount_df = sc.textFile(amount).map(
      line => {
        ...
        val arr = rline.split(",")
        if(arr.length==6){
          CaseClass.AMOUNT(arr(0).toInt, arr(1), arr(2).toInt, arr(3).toDouble, arr(4), arr(5))
        }else{
          CaseClass.AMOUNT(arr(0).toInt, arr(1), arr(2).toInt, arr(3).toDouble, arr(4), "")
        }
      }).toDS()
    ...
    val stt_object_df = sc.textFile(stt_object).map(
      line => {
        ...
        val arr = rline.split("\\|")
        if(arr.length==15) {
          CaseClass.STT_OBJECT(arr(0).toInt, arr(1), arr(2).toInt, arr(3), arr(4).toInt, arr(5),
            arr(6), arr(7), arr(8), arr(9), arr(10).toInt, arr(11), arr(12), arr(13), arr(14))
        }else{
          CaseClass.STT_OBJECT(arr(0).toInt, arr(1), arr(2).toInt, arr(3), arr(4).toInt, arr(5),
            arr(6), arr(7), arr(8), arr(9), arr(10).toInt, arr(11), arr(12), arr(13), "")
        }
      }).toDS()
    ...
    /*val sql = "select g.gen_id,g.ref_no,stt.cust_name,ait.acct_item_type_id,ait.name,bc.cycle_begin_date,bc.cycle_end_date," +
      "a.amount,a.currency,ait.pay_flag,g.start_date,g.end_date," +
      "decode(decode((case when instr(ait.name, '调整')==1 then 1 else -1 end), 1, ai.comments, at.acct_type), 'TELEPHONE', 1," +
      "decode(decode((case when instr(ait.name, '调整')==1 then 1 else -1 end), 1, ai.comments, at.acct_type), 'TELX', 2," +
      "decode(decode((case when instr(ait.name, '调整')==1 then 1 else -1 end), 1, ai.comments, at.acct_type), 'TELEGRAM', 3," +
      "decode(decode((case when instr(ait.name, '调整')==1 then 1 else -1 end), 1, ai.comments, at.acct_type), 'COMMITMENT ADJUST', 5," +
      "decode(decode((case when instr(ait.name, '调整')==1 then 1 else -1 end), 1, ai.comments, at.acct_type),ai.comments, 6, 4)))))  sort_id," +
      "decode((case when instr(ait.name, '调整')==1 then 1 else -1 end),1, ai.comments, at.acct_type) acct_name,g.acct_date " +
      "from gen_item g,acct_item_type ait,acct_item ai,amount a, stt_object stt,billing_cycle bc,account_type at" +
